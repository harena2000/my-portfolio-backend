  ...
  try {
    ...
    res.json({
      ...
      workExperience: workExperience.map((e) => ({
        id: e.id, type: e.type, companyOrSchool: e.companyOrSchool,
        roleOrDegree: e.roleOrDegree, location: e.location,
        startDate: e.startDate.toISOString().split("T")[0],
        endDate: e.endDate ? e.endDate.toISOString().split("T")[0] : null,
        isPresent: e.isPresent, description: e.description, logoUrl: e.logoUrl,
      })),
    ...
    if (certifications) {
      ...
      for (const cert of certifications) {
        await prisma.certification.create({
          data: { name: cert.name, issuer: cert.issuer || "", date: cert.date || "", url: cert.url || "" },
        });
      }
    }
...
router.delete("/pdf", authMiddleware, async (_req: Request, res: Response) => {
  try {
    ...
    console.error("Resume PDF delete error:", error);
    res.status(500).json({ message: "Internal server error", error: String(error) });
  }
});
